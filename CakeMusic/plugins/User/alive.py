...
async def alive(app: app, message: Message):
    ...
    try:
       ...
       pass
    send = app.send_video if alive_logo.endswith(".mp4") else app.send_photo
    xd = (f"{txt}")
    try:
...
async def repo(bot: app, message: Message):
    await message.edit("⚡")
...
@app.on_message(filters.command("creator", ".") & filters.me)
async def creator(bot: app, message: Message):
    await message.edit("https://gitHub.com/itz-zaid")


@app.on_message(filters.command(["uptime", "up"], ".") & filters.me)
async def uptime(bot: app, message: Message):
    now = datetime.now()
...
async def get_id(bot: app, message: Message):
    file_id = None
    user_id = None
