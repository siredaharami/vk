...
from CakeMusic import app, CMD_HELP
from CakeMusic.helper.test import ReplyCheck
from CakeMusic.helper.test2 import split_list
...
async def module_help(app: app, message: Message):
    cmd = message.command
    help_arg = ""
...
async def module_helper(app: app, message: Message):
    cmd = message.command
    help_arg = ""
    if len(cmd) > 1:
        help_arg = " ".join(cmd[1:])
